...
"""
This file can be used for generating different groups or "suites" of
benchmarks. A suite named with the suffix "strips" means that the FDR file
generated by the Fast Downward translator is SAS+ + action costs, while the
"adl" suffix means that it may also contain conditional effects and/or axioms.
These names are not ideal for distinguishing the domains, but they are the way
they are for historical reasons.
...
the regular IPC suites. They only figure in special suites such as

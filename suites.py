...
HELP = """This file can be used for converting suite names to lists of domains
or tasks, called suites. A suite named with the suffix "strips" means that the
FDR file generated by the Fast Downward translator is SAS+ plus action costs,
while the "adl" suffix means that it may also contain conditional effects
and/or axioms. These names are not ideal for distinguishing the domains, but
they are the way they are for historical reasons.

For some domains, there are several formulations. Suites with "ipc" in their
names (and their combinations) only contain one formulation of every domain. If
there are other formulations, they are in separate suites which are not part of
the regular IPC suites. They are only part of special suites such as
"suite_alternative_formulations" and "suite_all"."""
